...
void setup()
{
	...
	attachInterrupt(SPD_INT_L, Encoder_L, RISING); // pin 2
  	attachInterrupt(SPD_INT_R, Encoder_R, RISING); // pin 3	
...
//	setPoint = setPoint / 1000.0; 
	imu::Vector<3> euler = bno.getVector(Adafruit_BNO055::VECTOR_EULER);
		...
		{
			Speed_Avg = (Speed_L + Speed_R);
			pid.Compute(); // use pid loop to calculate output
			output = output + (kPwheel * Speed_Avg) + (kDwheel * (Speed_Avg - last_count));
			last_count = Speed_L;
		//	Serial.println(y_val);
			if (y_val > 0.4)
			{
				//double output1 = compensate_slack((-output - y_val), 1); //M1
				md.setM1Speed(-15); //26.5
				md.setM2Speed(0); // 25
...
*/
			...
			if (abs(left_stick) > 0.1) // we're supposed to be stationary
			{
			...
			else
			{
				...
				if (Speed_L > Speed_R)
				{
					md.setM1Speed(output1 - speed_dif);
					md.setM2Speed(-output2 - speed_dif);
				}
